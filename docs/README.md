...
- 当前新增设计文档：`./superpowers/specs/2026-06-30-design-to-frontend-project-delivery-profile.md`（将 design-to-frontend-delivery 的项目交付画像固化为设计转代码前置门禁：设计平台上下文、生成代码、导出 HTML/CSS 或 Figma-to-code 类工具产物只能作为参考；实现前必须记录 target stack/runtime、route/shell owner、component system、token/theme、styling、copy/i18n、assets、mock/API/BFF、directory/test/story 和 browser acceptance，并抽取 design intent map，把 semantic regions、source components/variants、重复组、交互状态和数据角色映射到宿主项目）
