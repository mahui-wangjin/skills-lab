...
- 自测路径：项目已有真实浏览器 E2E/agent-browser/Playwright/Cypress/browser/screenshot 命令，或无现成命令时的最小真实浏览器验收路径；smoke 只作健康检查
- 复刻范围与非目标项
...
默认用真实浏览器验收作为前端质量主门禁。优先运行项目已有 E2E、agent-browser、Playwright、Cypress、browser 或 screenshot 流程；smoke 只作为启动和关键路由健康检查。没有现成命令时，执行最小真实浏览器自测：
