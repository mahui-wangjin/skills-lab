...
3. 内容层：文案、图片、图标、按钮归属、标题标签
4. 交互层：点击反馈、展开收起、弹框开合、联动与返回路径
5. 校验层：必填/格式、错误提示、禁用态、提交与失败重试
6. 状态层：加载/空态/成功/失败/不可操作/角色差异
7. 表现层：字体、颜色、圆角、阴影、动效节奏、过渡一致性、反馈清晰度
...
- 已确认“1:1 / pixel-perfect / 像设计稿”只表示高保真目标，不表示按设计稿坐标绝对定位复刻
- 范围与非目标明确
- 壳层是否保留明确
...
- `convert-and-polish`：结构差异清单 + 布局策略与绝对定位审计 + 进入精修的页面批次
