...
后台页面必须先从用户任务、当前模板能力、右侧内容区线稿、基础页面元素、项目级页面范式、已确认页面基线、交互容器、字段/操作/状态治理出发，再进入编码。这个 skill 不绑定 Vben、Ant Design、Element Plus、MUI、React Admin、shadcn 或任何框架；它要求先现场盘点当前项目实际提供的能力，再基于这些能力建立或复用项目后台范式。
...
如果当前项目已经有用户确认过的后台页面基线、设计系统或实施方案，后续同类页面必须优先复用其结构、交互和验收口径，不得只拿通用原则重新发挥。项目内已确认反馈优先级高于本 skill 的通用默认建议。

任何可见控件、图标、按钮、帮助入口、工具栏入口和行操作都必须有明确可用行为。除非需求明确要求占位或禁用态，否则不能出现点了没反应、没有内容、没有权限反馈或只有视觉装饰的入口。不能用的功能要么实现，要么移除；确需保留的禁用入口必须有清楚的禁用原因。
...
- 全页面 UI 落地方案、设计规范、OpenSpec 或 ADR。
- 用户已经确认过的代表性页面或页面基线。
- 可复用的共享组件、列组件、详情抽屉、工作台 shell。
- 已确认页面产生过的用户负反馈和二次修正记录。

输出必须回答：

- 本页属于哪个已确认页面范式，或说明没有可用基线。
- 将复用哪个基线的结构、筛选策略、工具栏、字段分级、长文本处理和详情承载。
- 哪些地方因为业务不同需要变更，为什么。
- 哪些地方禁止再出现既有基线修正过的问题。

如果项目已有同类页面基线但没有回放，不得进入完整实现。
...
[可折叠适用备注/说明]
[Tabs，如需要]
[筛选区：高频条件 + 折叠低频条件 + 属性筛选/快速搜索；搜索策略需明确且不重复]
...
完成前必须做一次钢人反论审计：站在不满意用户、产品经理和长期维护者角度，主动找出为什么“看起来改了但没有变好”。如果发现主任务仍不清晰、控件重复、提示挤占空间、详情堆叠、表格行挤压或交互不符合用户习惯，先修正再交付。
...
表格是后台页面的主要工作面，不能让行内容把页面撑乱：

- 默认扫描列使用单行省略，保持行高稳定。
- 少数阅读型列表可使用两行省略；允许换行时行高必须自适应。
- 长说明、payload、trace、审计证据、原始 JSON 默认进入详情抽屉或详情页。
- 需要快速查看完整单元格内容时，用可聚焦/悬停的轻量浮层；不要在主表直接展开长文本。
- 如果项目已有共享表格文本组件，必须优先复用。
...
- 默认只展示高频条件。
- 低频条件进入折叠区域。
- 搜索策略必须二选一：自动筛选，或手动提交。
- 自动筛选适合轻量、低成本、结果反馈快的列表筛选：字段变化防抖提交，输入框回车提交，保留重置和收起，不再显示提交/搜索按钮，也不再显示 toolbar 搜索入口。
- 手动提交适合昂贵查询、高风险查询或需要组合条件后统一提交的页面：必须支持按钮提交和回车提交。
- 不得同时启用自动筛选、搜索按钮和 toolbar 搜索快捷入口，除非能证明三者职责不同。
- 搜索状态要能回显。
- 搜索区不能长期挤占主要工作区。
...
- 行操作是一长串按钮。
- 搜索区过高，挤占工作区。
- 自动筛选同时保留搜索按钮或 toolbar 搜索入口，形成重复功能。
- 手动搜索不支持回车提交。
- 标题旁、工具栏、行操作或帮助区域存在无效控件、空帮助、点了没反应的入口。
- 为了“看起来完整”放置暂不可用功能，但没有禁用态、权限反馈或生产边界说明。
- 常驻提示、说明、指标卡或帮助信息占据主工作区，却不直接服务当前任务。
- 详情抽屉顶部继续堆大块摘要、说明、证据，导致用户仍像在主页面长滚动。
- 表格长文本只做省略但无法快速查看，也没有进入详情承载。
...
- 长文本、多列、窄屏不会破坏布局。
- 没有重复控件、无效帮助入口、常驻大提示、详情堆叠、行内容挤压。
- 所有可见控件和入口都有可验证行为；暂不可用入口有禁用态和原因，否则已移除。
- 搜索策略符合用户预期：自动筛选不显示重复搜索入口；手动搜索支持回车。
- 重要后台流程使用真实浏览器或截图验收。
