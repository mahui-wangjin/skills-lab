...
该 skill 现在新增“项目交付画像与设计语义图”门禁：Figma-to-code 类工具、生成 JSX/Vue/HTML、导出 HTML/CSS 或 Dev Mode/MCP 代码片段只能作为参考材料，不是项目架构合同。写代码前必须记录 target stack/runtime、route/shell owner、component system、token/theme、styling policy、copy/i18n、asset/icon/font、mock/API/BFF、directory/test/story 和 browser acceptance path，并抽取 design intent map，把 semantic regions、source components/variants、重复组、交互状态、数据角色和 missing facts 映射到宿主项目。不得因为生成代码里出现某个 UI library、CSS reset、locale shape、fixture schema、API client、route shell 或 test harness，就把它带进真实项目。
...
用于在前端、后端、worker、脚本、测试辅助、smoke/E2E/browser automation 和验证脚本等代码继续堆功能前做可维护性守门。它不按行数洁癖强拆，也不把 800 行当作唯一触发点；每次改代码都先做轻量边界检查，再按职责分离、关注点分离、高内聚低耦合、单一抽象层级、变化轴、复用/重复压力、测试边界、依赖方向和稳定接口判断是否允许继续原地开发、需要小步拆分，或必须先做结构恢复。测试代码也是代码：runner/bootstrap、fixtures、selectors/actions、assertions、evidence capture、reporting 和 cleanup 不能长期堆在一个巨型脚本里；AI 驱动 browser/E2E 也要有目标流程、视口、证据、停止条件和必要的验收预算确认，不能变成无边界试错。
...
- `Figma/其他设计平台链接 + 已安装插件或 MCP`：应先读取平台可提供的结构、样式、tokens、组件映射或 reference code。Figma 要优先用目标 node/selection/frame 的 Dev Mode/MCP/Code Connect 上下文；不得把下载整张设计图、整页 canvas 截图或全文件 raster 当作第一步或主基线。
- `FigmaToCodePro/其他设计转代码工具已经生成了 React 代码`：应把生成代码当作 source-backed draft，先建立 project delivery profile 和 design intent map，再把结构、copy、样式事实映射到宿主项目的 components、tokens、route shell、i18n、assets、fixtures/API boundary 和 tests；不得直接复制工具自带的 UI library、token scale、locale key、mock API、route shell 或 test harness。
...
- `改完页面后准备交付`：应运行真实浏览器 E2E、agent-browser、Playwright、Cypress、browser automation、浏览器驱动截图/trace 或 Storybook-in-browser 验收路径，或执行最小真实浏览器自测；覆盖主流程、层级、弹层、桌面/移动、控制台错误、失败请求和资源加载。smoke 只证明健康检查，不能替代产品体验验收。没跑真实浏览器验收就只能说 conditional/self-reviewed/代码级候选结果，并提醒用户缺少哪些浏览器证据。
- `精修阶段要不要让 AI 做完整端到端验收`：应区分最小真实浏览器验收和高成本 AI 探索式 E2E。最小验收可直接执行；跨多路由/多视口/多轮视觉 diff/大范围回归/登录或第三方流程等高成本验收，需要先给出覆盖范围、预计时间或 token 成本、停止条件和人工复核项，请用户确认后再跑。
