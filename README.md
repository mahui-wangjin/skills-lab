...
用于设计、实现或审查后台管理界面、中后台、admin panel、CRUD 列表、配置控制台、审批/流程页、图谱/画布工作台、审计观测页等场景。它不绑定 Vben、Ant Design、Element Plus、MUI、React Admin 或任何框架，而是要求先盘点当前模板/框架的组件与完整页面示例，确认业务内容区线稿和基础页面元素，再归纳或复用项目级后台页面范式，确认后再编码。
